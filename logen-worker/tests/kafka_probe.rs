...
//!
//! ```text
//! cargo test -p logen-worker --test kafka_probe
//! cargo test -p logen-worker --test kafka_probe print_workspace_tls_assets_dir -- --nocapture
//! ```
...
fn kafka_line_sink_try_new_with_jks_fixture() {
    ...
    KafkaLineSink::new(&k).expect("create Kafka sink with JKS fixture");
}
