//! Shared configuration (TOML), embedded reference defaults, and [`LogenError`].
//!
...
/// 若命令行未出现 `--defaults-file` 且设置了环境变量 **`LOGEN_DEFAULTS_FILE`**（非空路径），
/// 则等价于在参数最前面插入 `--defaults-file <path>`，便于 `logen` / `logend` 与示例 TOML 对齐。
fn inject_defaults_file_from_env(args: Vec<String>) -> Vec<String> {
    if args.iter().any(|a| a == "--defaults-file") {
        return args;
    }
    let Ok(p) = std::env::var("LOGEN_DEFAULTS_FILE") else {
        return args;
    };
    let p = p.trim();
    if p.is_empty() {
...
pub fn parse_cli_args(args: Vec<String>) -> Result<(Option<PathBuf>, Vec<String>), LogenError> {
    ...
    while i < args.len() {
        if args[i] == "--defaults-file" {
            let path = args
                .get(i + 1)
                .ok_or_else(|| {
                    LogenError::Cli("--defaults-file must be followed by a path".into())
                })?
                .clone();
...
mod tests {
    ...
    /// 测试内容：`--defaults-file` 覆盖环境变量 `LOGEN_DEFAULTS_FILE`。
    /// 输入：先设 `LOGEN_DEFAULTS_FILE=/tmp/x.toml` 解析 `ping`；再带长选项 `--defaults-file /tmp/y.toml`。
    /// 预期：无 flag 时默认路径为 `/tmp/x.toml`；有 flag 时为 `/tmp/y.toml`；剩余参数均为 `ping`；最后清除环境变量。
    #[test]
    fn defaults_file_env_and_flag_precedence() {
        std::env::set_var("LOGEN_DEFAULTS_FILE", "/tmp/x.toml");
        ...
        std::env::remove_var("LOGEN_DEFAULTS_FILE");
    }
}
