//! 声明式模板造日志：**`template`** + **`fields`**；**`sink:`** 嵌套块（**`type`**：`kafka` | `file` | `stdout`）。**`output`** 仅 **`file`** 需要；**`kafka`** 配 **`sink.kafka:`**（含可选 **`headers:`** 记录头）。**`sink.max-size`** 可为整数（字节）或字符串（如 **`64KiB`**、**`10MiB`**，1024 进制）。
...
pub use runner::{
    ...
    KafkaPassthroughFields, LineSinkType, ProducerConfigLayer, SinkConfig, TemplateConfig,
    TemplateRunner,
};
