...
CLI 本身不渲染日志、不写 Kafka；它把**实例 YAML**交给 daemon，由进程内的 **`logen-worker`** 执行。YAML 语法见 **[logen-dsl 规范](../../guide/book/logen-dsl/intro.html)**（仓库根 [`guide/`](../../../guide/)）。
...
2. 编写实例 YAML（见 [logen-dsl](../../guide/book/logen-dsl/intro.html) 或仓库 [`etc/`](../../../etc/) 示例）。
...
**logen-dsl** 配置说明在仓库根 **`guide/`**（`cd guide && mdbook build`）。本书仅描述 CLI / daemon。

HTML 输出在 **`logen/guide/book/`**（已 `.gitignore`）。含 Mermaid 时需 **`mdbook-mermaid`** 与 **`./scripts/fetch-mdbook-mermaid-assets.sh`**。
