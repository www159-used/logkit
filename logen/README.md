...
Producer 语法见 [`logen-dsl` 规范](../logen-dsl/guide/src/intro.md)；示例文件在 [**`etc/`**](../etc/)。HTML：`cd ../logen-dsl/guide && mdbook build`。
...
- producer YAML 作为单次 RPC 载荷；必要时调大 TOML 里 `[protocol.grpc]` 的 **`max_encoding_message_size_bytes`** 等（见 [`logen-config`](../logen-config/README.md)）。
