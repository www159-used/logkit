...
**控制面守护进程**：在 **Unix 套接字**上提供 **gRPC**（[`logspout-proto`](../logspout-proto/README.md)），在进程内通过 **`logspout-worker`** 库启动造日志任务。
...
| `{tmp_dir}/logspout-daemon.log` | 运行日志（`log` + **flexi_logger**，**追加**写入） |
...
- 示例：`RUST_LOG=debug`、`RUST_LOG=logspout_daemon=trace`（**Ping**、**Heartbeat** 等为 `trace`，避免默认级别刷盘过快）。
