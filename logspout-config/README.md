...
**共用 TOML**：内嵌一份参考默认配置，与用户 **`--defaults-file`**（或环境变量 **`LOGSPOUT_DEFAULTS_FILE`**）**深度合并**，后者覆盖前者。

公开入口：**`load_merged`**、**`LogspoutConfig`**、**`LogspoutError`** 等。

## 指定配置文件

| 方式 | 说明 |
|------|------|
| `--defaults-file PATH` | **`logspout`** / **`logspout-daemon`** 命令行 |
| `LOGSPOUT_DEFAULTS_FILE` | 非空路径时，等价于在 argv 最前插入 `--defaults-file`（见 crate 内 `inject_defaults_file_from_env`） |
...
| `ping_reply_text` | `logspout ping` 返回值。 |
...
| `heartbeat_timeout_secs` / `heartbeat_interval_secs` | 心跳与健康判定（见 `logspout list` / `stat`）。 |
...
- 客户端与 daemon：**[`logspout`](../logspout/README.md)**、**[`logspout-daemon`](../logspout-daemon/README.md)**  
- Producer 语法：**[`logspout-dsl`](../logspout-dsl/README.md)**  
