...
Rust 实现的日志造数 / 灌流工具链（CS 架构）：**Unix 套接字上的 gRPC 控制面** + 进程内 **`logspout-worker`** 执行造数。起因包含 ARM 产物、交叉编译与避免裸露 TCP 端口等考量。
...
| [`logspout-worker`](logspout-worker/README.md) | 造日志库（由 daemon 进程内嵌入） |
| [`logspout-dsl`](logspout-dsl/README.md) | Worker 模板 YAML：`template` / `fields` / `sink` |
...
产物：`target/release/logspout`、`logspout-daemon`。
...
4. [`logspout-dsl/README.md`](logspout-dsl/README.md)→ 编写 worker 模板 YAML  
