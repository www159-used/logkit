...
**总览与架构（mdBook）**：仓库根目录 [`guide/`](guide/)，`cd guide && mdbook build`（可选 `--open`），生成物在 `guide/book/`（已 `.gitignore`）。书中链到 DSL 构建产物 **`logen-dsl/guide/book/index.html`**（源在 [`logen-dsl/guide`](logen-dsl/guide)，需先在该目录执行 `mdbook build`）及各 crate `README`。
...
| Crate | 说明 |
|--------|------|
| [`logen`](logen/README.md) | CLI：`ping` / `start` / `list` / `stop` / `stat` / `cat` |
